...
// to be copyable and lightweight.
    struct Transition {
        enum Type {
            Sibling, Inner, InnerEntry, No
        };

        // Default is no transition
        Transition()
                : mTransitionType(Transition::No), mStateFactory(0) {
        }
...
// SiblingTransition
    ...
    Transition SiblingTransition(Args &&... args) {
        return Transition(Transition::Sibling, GetStateFactory<TargetState>(),
                          detail::GenerateOnEnterArgsFunc<TargetState>(std::forward<Args>(args)...));
    }
...
// State
    ...
    namespace detail {
        void
        InitState(State *state, StateMachine *ownerStateMachine, size_t stackDepth, const StateFactory &stateFactory);
    }

    struct State {
        State()
                : mOwnerStateMachine(0), mStackDepth(0), mStateValueResetters(0), mStateDebugName(0) {
        }
...
// manages a stack of states.
    ...
    public:
        ...
        const State *GetState(StateTypeId stateType) const {
            return const_cast<const State *>( const_cast<StateMachine *>(this)->GetState(stateType));
        }
...
// Inline State member function implementations - implemented here because they depend StateMachine being defined
    ...
    StateType *State::GetOuterState() {
        return static_cast<StateType *>(GetStateMachine().GetOuterState(hsm::GetStateType<StateType>(),
                                                                        mStackDepth - 1));
    }
    ...
    StateType *State::GetInnerState() {
        return static_cast<StateType *>(GetStateMachine().GetInnerState(hsm::GetStateType<StateType>(),
                                                                        mStackDepth + 1));
    }
    ...
    inline StateType *State::GetImmediateInnerState() {
        return static_cast<StateType *>(GetStateMachine().GetStateAtDepth(mStackDepth + 1,
                                                                          hsm::GetStateType<StateType>()));
    }
    ...
    inline void StateMachine::Log(size_t minLevel, size_t numSpaces, const hsm_char *format, ...) {
        if (static_cast<size_t>(mDebugTraceLevel) >= minLevel) {
            ...
            int offset = SNPRINTF(buffer, sizeof(buffer), HSM_TEXT("HSM_%lu_%s:%*s "),
                                  static_cast<unsigned long>(minLevel), mDebugName, static_cast<int>(numSpaces), "");
